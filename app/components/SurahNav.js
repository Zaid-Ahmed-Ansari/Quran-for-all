...
const handleCopyLink = () => {
  ...
  return (
    <div className="relative z-50">
      {/* Navbar */}
      <div className="flex w-screen border border-gray-200 divide-x divide-gray-200 text-sm h-[66.17px] text-gray-800 bg-white">
        {/* Chapter */}
        <button onClick={() => setShowChapterDropdown(!showChapterDropdown)} className="flex items-center justify-center flex-3 px-4 w-[300px] py-2 hover:bg-gray-100 gap-1">
        ...
        {/* Verse */}
        <button onClick={() => setShowVerseDropdown(!showVerseDropdown)} className="flex items-center justify-center flex-1 px-4 py-2 hover:bg-gray-100 gap-1">
          <span>Verse</span>
          <ChevronDown size={16} className="text-gray-600" />
        </button>

        {/* Language */}
        <button onClick={() => setShowLanguageDropdown(!showLanguageDropdown)} className="flex flex-1 justify-center items-center px-4 py-2 hover:bg-gray-100 gap-1">
          <span>Language</span>
          <ChevronDown size={16} className="text-gray-600" />
        </button>

        {/* Arabic */}
        <button onClick={() => setShowArabicDropdown(!showArabicDropdown)} className="flex flex-1 justify-center items-center px-4 py-2 hover:bg-gray-100 gap-1">
          <span>Arabic</span>
          <ChevronDown size={16} className="text-gray-600" />
        </button>

        {/* Settings */}
        <button onClick={() => setShowSettingsDropdown(!showSettingsDropdown)} className="flex flex-1 justify-center items-center px-4 py-2 hover:bg-gray-100 gap-1">
          <Image src='/settingssurahnav.svg' width={21} height={18} alt='settings' />
          <span>Settings</span>
        </button>

        {/* Share */}
        <button onClick={() => setShowShareModal(true)} className="flex flex-1 justify-center items-center px-4 py-2 hover:bg-gray-100 gap-1">
          <Image src='/share.svg' width={18} height={18} alt='share' />
          <span>Share</span>
        </button>

        {/* Search */}
        <div className='flex flex-2 justify-center items-center px-4 py-2 hover:bg-gray-100 gap-1'>
      ...
      {showChapterDropdown && (
        <div ref={dropdownRefs.chapter} className="absolute top-[68px] left-[20px] w-[400px] bg-white border rounded shadow max-h-[400px] overflow-y-auto p-2">
          ...
          <div className="text-sm text-text-primary font-lato px-2">Introduction of the Quran</div>
          {filteredSurahs.map((s) => (
            <Link key={s.number} href={`/verses/${s.number}/${s.englishName}`} className="flex justify-between group items-center px-4 py-2 hover:bg-navbg">
              <div>
                <p className="text-text-primary font-lato group-hover:text-hoverclr">{s.number}. {s.englishName}</p>
        ...
        <div ref={dropdownRefs.verse} className="absolute top-[68px] left-[300px] w-[260px] bg-white border rounded shadow p-2 z-50">
          ...
          <div className="grid grid-cols-5 gap-2 max-h-[200px] overflow-y-auto">
            {filteredVerses.map((v) => (
              <button key={v} onClick={() => alert(`Go to verse ${v}`)} className="w-10 h-10 flex items-center justify-center text-text-primary rounded-full hover:bg-hoverclr">{v}</button>
            ))}
          </div>
        </div>
      )}
